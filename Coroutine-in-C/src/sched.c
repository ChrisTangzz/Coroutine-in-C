...
static RBTREE_CMP_Pri_INSERT_DEFINE(rb_cmp_Pri_insert, _n1, _n2)
{
    struct task_struct *n1 = container_of(_n1, struct task_struct, node);
    struct task_struct *n2 = container_of(_n2, struct task_struct, node);
    if (n1->pritority < n2->pritority)
        return 1;
    else {
        if (n1->pritority == n2->pritority)
            n1->pritority++;
...
// static RBTREE_CMP_Pri_SEARCH_DEFINE(rb_cmp_Pri_search, _n1, key)
// {
//     struct task_struct *n1 = container_of(_n1, struct task_struct, node);

//     if (n1->pritority == *(long *)key)
//         return RB_EQUAL;
//     else if (n1->pritority > *(long *)key)
//         return RB_RIGHT;
//     else
//         return RB_LEFT;
// }
...
static inline int Priority_schedule(struct cr *cr, job_t func, void *args)
{
    ...
    new_task->context.wait_yield = 1;
    new_task->context.blocked = 1;
    new_task->pritority = rand();
    printf("%ld\n", new_task->pritority);
    rbtree_insert(&cr->root, &new_task->node, rb_cmp_Pri_insert);
    return new_task->tfd;
}

static inline struct task_struct *Pri_pick_next_task(struct cr *cr)
{
    struct rb_node *node = rbtree_min(&cr->root);
    struct task_struct *task = container_of(node, struct task_struct, node);
    struct timespec start;
...
static inline int Pri_put_prev_task(struct cr *cr, struct task_struct *prev)
{
    struct timespec end;

    clock_gettime(CLOCK_MONOTONIC, &end);
    prev->sum_exec_runtime += time_diff(prev->exec_start, end.tv_nsec);
    rbtree_insert(&cr->root, &prev->node, rb_cmp_Pri_insert);
...
void sched_init(struct cr *cr)
{
    ...
    case CR_PRIORITY:
        ...
        cr->pick_next_task = Pri_pick_next_task;
        cr->put_prev_task = Pri_put_prev_task;
        return;
    }
}
